...
public class UTF8StringUtil {
    ...
    public static int lowerCaseCompareTo(byte[] thisBytes, int thisStart, int thisLength, byte[] thatBytes,
        ...
        return compareTo(thisBytes, thisStart, thisLength, thatBytes, thatStart, thatLength, false, true, false);
    }
    ...
    public static int lowerCaseHash(byte[] bytes, int start) {
        return hash(bytes, start, -1, true, true, false, 31, Integer.MAX_VALUE);
    }

    // Certain type of string does not include lengthByte in the beginning and
    // the length of the given string is given explicitly as a parameter.
    public static int lowerCaseHash(byte[] bytes, int start, int length) {
        return hash(bytes, start, length, false, true, false, 31, Integer.MAX_VALUE);
    }
    ...
    private static int compareTo(byte[] thisBytes, int thisStart, byte[] thatBytes, int thatStart,
        ...
        return compareTo(thisBytes, thisStart, -1, thatBytes, thatStart, -1, true, useLowerCase, useRawByte);
    }

    private static int compareTo(byte[] thisBytes, int thisStart, int thisLength, byte[] thatBytes, int thatStart,
            int thatLength, boolean lengthByteIncluded, boolean useLowerCase, boolean useRawByte) {
        int utflen1;
        int utflen2;
        int s1Start;
        int s2Start;

        if (lengthByteIncluded) {
            utflen1 = getUTFLength(thisBytes, thisStart);
            utflen2 = getUTFLength(thatBytes, thatStart);
            s1Start = thisStart + getNumBytesToStoreLength(utflen1);
            s2Start = thatStart + getNumBytesToStoreLength(utflen2);
        } else {
            utflen1 = thisLength;
            utflen2 = thatLength;
            s1Start = thisStart;
    ...
    private static int hash(byte[] bytes, int start, boolean useLowerCase, boolean useRawByte, int coefficient, int r) {
        return hash(bytes, start, -1, true, useLowerCase, useRawByte, coefficient, r);
    }

    // In certain type of string, length can be given explicitly.
    private static int hash(byte[] bytes, int start, int length, boolean lengthByteIncluded, boolean useLowerCase,
        ...
        // the length of the given string is given explicitly as a parameter.
        if (lengthByteIncluded) {
            utflen = getUTFLength(bytes, start);
            sStart = start + getNumBytesToStoreLength(utflen);
        } else {
            utflen = length;
            sStart = start;
        }
