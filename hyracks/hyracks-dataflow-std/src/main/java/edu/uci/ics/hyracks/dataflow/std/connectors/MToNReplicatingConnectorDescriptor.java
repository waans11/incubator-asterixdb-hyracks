...
public class MToNReplicatingConnectorDescriptor extends
	...
	public IFrameWriter createPartitioner(IHyracksTaskContext ctx,
			RecordDescriptor recordDesc, IPartitionWriterFactory edwFactory,
		...
		final String nodeJobSignature = ctx.getJobletContext()
				.getApplicationContext().getNodeId()
				+ ctx.getJobletContext().getJobId()
				+ ctx.getJobletContext().hashCode();
		final String taskId = ctx.getTaskAttemptId()
				+ this.toString();
			...
			public void close() throws HyracksDataException {
				...
				if (ExperimentProfiler.PROFILE_MODE) {
					...
					OperatorExecutionTimeProfiler.INSTANCE.executionTimeProfiler
							.add(nodeJobSignature, taskIdWithTimeStamp,
									profilerSW.getMessage(
											"MToNReplicatingConnector",
											profilerSW.getStartTimeStamp()),
									false);
					System.out.println("MToNReplicatingConnector end "
							+ taskIdWithTimeStamp);
			...
			public void open() throws HyracksDataException {
				...
				if (ExperimentProfiler.PROFILE_MODE) {
					...
					taskIdWithTimeStamp = taskId
							+ profilerSW.getStartTimeStamp();
					OperatorExecutionTimeProfiler.INSTANCE.executionTimeProfiler
							.add(nodeJobSignature, taskIdWithTimeStamp, "init",
									false);
					System.out.println("MToNReplicatingConnector start "
							+ taskIdWithTimeStamp);
				}
				for (int i = 0; i < epWriters.length; ++i) {
					epWriters[i].open();
